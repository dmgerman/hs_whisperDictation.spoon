#!/bin/bash
# WhisperServerTranscriber Live Integration Test
# Tests WhisperServer transcriber with Manager + SoxRecorder via spoon interface
...
test_suite "WhisperServer Transcriber Integration (via Spoon Interface)"
...
assert_command_exists curl && pass

test_case "Hammerspoon is running"
assert_hs_running && pass

# Note: This test requires a running whisper server
# Start server with: python whisper_server.py --port 8080
test_case "whisper server is running (optional)"
if curl -s http://127.0.0.1:8080/health > /dev/null 2>&1; then
  pass
else
  skip "Whisper server not running on port 8080 (start with: python whisper_server.py)"
fi
...
TEST_TEMP_DIR="/tmp/whisperserver_test_$$"
...
# ==============================================================================
# Load Spoon with WhisperServer Transcriber
# ==============================================================================

test_case "can load spoon with WhisperServer transcriber"
...
test_case "WhisperServer transcriber validates curl"
...
assert_equals "WhisperServer" "$NAME" && pass
...
# ==============================================================================
# Transcription (Server should be fast, ~5-10 seconds)
# ==============================================================================

test_case "WhisperServer transcribes audio"
MAX_WAIT=30
ELAPSED=0
