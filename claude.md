...
- 2026-02-15: **StreamingRecorder persistent server pattern** - Step 8b implementation uses persistent Python server (start once, reuse for multiple recordings) with explicit cleanup() method called by init.lua on spoon unload. Server stays running across recordings, reducing startup latency. Graceful shutdown via TCP "shutdown" command before process termination. See `recorders/streaming/streaming_recorder.lua` for implementation.
- 2026-02-15: **Critical Manager stopRecording() bug in Step 8b** - Manager:stopRecording() was unconditionally transitioning to TRANSCRIBING state, assuming chunks hadn't been emitted yet (true for SoxRecorder, FALSE for StreamingRecorder where chunks emit during recording). Fixed by adding `self:_checkIfComplete()` call after transition to TRANSCRIBING. This immediately transitions to IDLE if pendingTranscriptions == 0. Without this fix, Manager stays stuck in TRANSCRIBING state forever when StreamingRecorder emits all chunks before stopRecording() is called.
- 2026-02-15: **Multi-chunk coordination during recording** - StreamingRecorder emits chunks DURING recording (not just at stop like SoxRecorder). Manager handles this via callback-based coordination: (1) _onChunkReceived() increments pendingTranscriptions, starts async transcription, (2) transcription callbacks decrement counter and call _checkIfComplete(), (3) stopRecording() marks recordingComplete=true and calls _checkIfComplete(), (4) completion only when BOTH recordingComplete==true AND pendingTranscriptions==0. This enables parallel transcription while still recording.
- 2026-02-15: **Python server resilience on microphone failures** - whisper_stream.py must STAY RUNNING when microphone fails (not crash). Wrapped command loop in infinite retry with exception handling. Server sends error events via TCP, then continues waiting for commands. Test validates: server timeout (stays running) instead of exit code. This prevents needing server restart for transient microphone failures. Critical for reliability.
- 2026-02-15: **All parameters configurable from command line** - Step 8b lesson: Don't hardcode ANY timing/behavior parameters. Make everything configurable via command-line arguments passed from Lua config. Examples: --silence-threshold, --min-chunk-duration, --max-chunk-duration, --perfect-silence-duration, --audio-input. This enables testing edge cases and user customization without code changes.
- 2026-02-15: **Perfect silence detection default disabled** - perfectSilenceDuration parameter defaults to 0 (disabled) in production. When enabled (e.g., 2.0s for testing), it can incorrectly detect silence before audio starts and stop recording early, preventing chunk creation. Only enable when explicitly testing microphone silence detection, not for normal use. Configurable via --perfect-silence-duration parameter.
- 2026-02-15: **State transition bug with StreamingRecorder** - Manager's `_finalize()` tried to transition RECORDING → IDLE, which is invalid (RECORDING can only go to TRANSCRIBING or ERROR). Bug only manifested in production with async transcription, not in tests with synchronous mocks. Fixed by checking current state before transitioning in `_finalize()`. Added test to validate no error alerts occur during successful recording. **Lesson**: Always test that NO errors occurred, not just that final state is correct.
- 2026-02-15: **Whisper hallucination on silence** - Whisper models generate phantom transcriptions (commonly "Thank you", "Thanks for watching", etc.) when given silence or no audio input. This is a known bug in Whisper, not our code. Filter or warn users when suspicious patterns detected.
